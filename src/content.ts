...
export const RESEARCH=[
 ...
 {id:'q17',sp:'anvil', nm:{ko:'장비 연성',en:"Gear Transmutation"},   cost:1e16, d:()=>X('장비 효과 지수 ×1.5',"Gear effect exponent ×1.5"), apply:m=>m.gearExp*=1.5},
...
/* 룬 · 오퍼링으로 강화, 승천 시 초기화 */
export const RUNES=[
 {id:'speed', sp:'rune_speed', nm:{ko:'신속의 룬',en:"Rune of Haste"}, d:(l,n=l+1)=>`${X('게임 속도',"Game speed")} +${pctTxt(1.5*l)}% → +${pctTxt(1.5*(n))}%`,        apply:(m,l)=>m.speed*=1+0.015*l},
 {id:'wealth',sp:'rune_wealth',nm:{ko:'풍요의 룬',en:"Rune of Plenty"}, d:(l,n=l+1)=>`${X('마나 생산',"Mana output")} ×${powTxt(1.06,l)} → ×${powTxt(1.06,n)}`,      apply:(m,l)=>m.prod*=Math.pow(1.06,l)},
 {id:'wisdom',sp:'rune_wisdom',nm:{ko:'지혜의 룬',en:"Rune of Wisdom"}, d:(l,n=l+1)=>`${X('오퍼링 획득',"Offerings")} +${fmt((8*l))}% → +${fmt((8*(n)))}%`,                   apply:(m,l)=>m.offer*=1+0.08*l},
 {id:'guard', sp:'rune_guard', nm:{ko:'수호의 룬',en:"Rune of Warding"}, d:(l,n=l+1)=>`${X('던전 공격력',"Dungeon power")} ×${powTxt(1.12,l)} → ×${powTxt(1.12,n)}`,    apply:(m,l)=>m.dungeon*=Math.pow(1.12,l)},
 {id:'abyss', sp:'rune_abyss', nm:{ko:'심연의 룬',en:"Rune of the Abyss"}, d:(l,n=l+1)=>`${X('영혼석 획득',"Soul Shards")} ×${powTxt(1.08,l)} → ×${powTxt(1.08,n)}`,    apply:(m,l)=>m.soul*=Math.pow(1.08,l)},
];
RUNES.push(
 {id:'might', sp:'rune_might', nm:{ko:'맹위의 룬',en:"Rune of Fury"},
  d:(l,n=l+1)=>`${X('던전 보상',"Dungeon rewards")} ×${powTxt(1.10,l)} → ×${powTxt(1.10,n)}`, apply:(m,l)=>m.floorLoot*=Math.pow(1.10,l)},
 {id:'mind',  sp:'rune_mind',  nm:{ko:'사색의 룬',en:"Rune of Contemplation"},
  d:(l,n=l+1)=>`${X('아래 두 단계 효율',"Bottom two tiers")} ×${powTxt(1.09,l)} → ×${powTxt(1.09,n)}`, apply:(m,l)=>m.t0*=Math.pow(1.09,l)},
 {id:'flow',  sp:'rune_flow',  nm:{ko:'물결의 룬',en:"Rune of Currents"},
  d:(l,n=l+1)=>`${X('자동화 주기',"Automation interval")} ×${cutTxt(0.985,l)} → ×${cutTxt(0.985,n)}`, apply:(m,l)=>m.autoSpeed*=Math.pow(0.985,l)},
 {id:'hollow',sp:'rune_void',  nm:{ko:'공동의 룬',en:"Rune of the Hollow"},
  d:(l,n=l+1)=>`${X('보스 보상',"Boss rewards")} ×${powTxt(1.11,l)} → ×${powTxt(1.11,n)}`, apply:(m,l)=>m.boss*=Math.pow(1.11,l)},
 {id:'gate',  sp:'rune_gate',  nm:{ko:'관문의 룬',en:"Rune of Gates"},
  d:(l,n=l+1)=>`${X('탐사 깊이 배율',"Depth multiplier")} ×${powTxt(1.07,l)} → ×${powTxt(1.07,n)}`, apply:(m,l)=>m.floorPct*=Math.pow(1.07,l)},
 {id:'temper',sp:'rune_forge', nm:{ko:'담금질의 룬',en:"Rune of Tempering"},
  d:(l,n=l+1)=>`${X('장비 효과 지수',"Gear exponent")} ×${powTxt(1.05,l)} → ×${powTxt(1.05,n)}`, apply:(m,l)=>m.gearExp*=Math.pow(1.05,l)},
 {id:'echo',  sp:'rune_echo',  nm:{ko:'메아리의 룬',en:"Rune of Echoes"},
  d:(l,n=l+1)=>`${X('도전 보상',"Trial rewards")} ×${powTxt(1.08,l)} → ×${powTxt(1.08,n)}`, apply:(m,l)=>m.chalPow*=Math.pow(1.08,l)},
);
...
export const SOUL_UPS=[
 {id:'s1', sp:'gem',      nm:{ko:'영혼 각인',en:"Soul Sigil"},  max:Infinity,c:l=>2*Math.pow(1.7,l),  d:(l,n=l+1)=>`${X('마나 생산',"Mana output")} ×${powTxt(1.15,l)} → ×${powTxt(1.15,n)}`, apply:(m,l)=>m.prod*=Math.pow(1.15,l)},
 {id:'s2', sp:'coinpurse',   nm:{ko:'값싼 주문서',en:"Cheap Scrolls"},max:Infinity,      c:l=>6*Math.pow(2.1,l),  d:(l,n=l+1)=>`${X('시설 비용 증가율',"Cost growth")} ×${cutTxt(0.93,l)} → ×${cutTxt(0.93,n)}`, apply:(m,l)=>m.costMul*=Math.pow(0.93,l)},
 {id:'s3', sp:'fastfwd',  nm:{ko:'신속한 손길',en:"Swift Hands"},max:Infinity,c:l=>10*Math.pow(2.1,l), d:(l,n=l+1)=>`${X('자동화 주기',"Automation interval")} ${pctTxt(Math.pow(0.88,l)*100)}% → ${pctTxt(Math.pow(0.88,n)*100)}%`, apply:(m,l)=>m.autoSpeed*=Math.pow(0.88,l)},
 {id:'s4', sp:'sparkle',  nm:{ko:'영혼 공명',en:"Soul Resonance"},  max:Infinity,c:l=>14*Math.pow(2.2,l), d:(l,n=l+1)=>`${X('영혼석 획득',"Soul Shards")} ×${powTxt(1.3,l)} → ×${powTxt(1.3,n)}`, apply:(m,l)=>m.soul*=Math.pow(1.3,l)},
 {id:'s5', sp:'mastery',  nm:{ko:'던전 숙련',en:"Dungeon Mastery"},  max:Infinity,c:l=>9*Math.pow(1.95,l), d:(l,n=l+1)=>`${X('던전 공격력',"Dungeon power")} ×${powTxt(1.35,l)} → ×${powTxt(1.35,n)}`, apply:(m,l)=>m.dungeon*=Math.pow(1.35,l)},
 {id:'s6', sp:'moon',     nm:{ko:'오프라인 계약',en:"Offline Pact"},max:Infinity,    c:l=>18*Math.pow(1.8,l), d:(l,n=l+1)=>`${X('오프라인 상한',"Offline cap")} ${fmt(4+2*l)}${X("시간","h")} → ${fmt(4+2*n)}${X("시간","h")}`, apply:(m,l)=>m.offline=4+2*l},
 {id:'s7', sp:'gift',     nm:{ko:'시작 지원금',en:"Starting Grant"},max:Infinity,c:l=>22*Math.pow(2.6,l), d:(l,n=l+1)=>`${X('환생 직후 마나',"Mana right after rebirth")} ${fmtLog(startManaLog(l))} → ${fmtLog(startManaLog(n))}`, apply:()=>{}},
 {id:'s8', sp:'runering',nm:{ko:'룬 친화',en:"Rune Affinity"}, max:Infinity,       c:l=>25*Math.pow(2.4,l), d:(l,n=l+1)=>`${X('룬 최대 레벨',"Rune level cap")} +${fmt(5*l)} → +${fmt(5*(n))}`, apply:(m,l)=>m.runeCap+=5*l},
 {id:'s9', sp:'chisel',  nm:{ko:'결정 세공',en:"Crystal Cutting"},  max:Infinity,c:l=>16*Math.pow(2.3,l), d:(l,n=l+1)=>`${X('결정 획득',"Crystals")} ×${powTxt(1.5,l)} → ×${powTxt(1.5,n)}`, apply:(m,l)=>m.crystal*=Math.pow(1.5,l)},
 {id:'s10',sp:'chalice', nm:{ko:'봉헌의 축복',en:"Blessing of Offering"},max:Infinity,c:l=>20*Math.pow(2.35,l),d:(l,n=l+1)=>`${X('오퍼링 획득',"Offerings")} ×${powTxt(1.4,l)} → ×${powTxt(1.4,n)}`, apply:(m,l)=>m.offer*=Math.pow(1.4,l)},
];

SOUL_UPS.push(
 {id:'s11',sp:'bulwark',   nm:{ko:'영혼 방벽',en:"Soul Bulwark"}, max:Infinity,c:l=>18*Math.pow(2.4,l),
  d:(l,n=l+1)=>`${X('던전 보상',"Dungeon rewards")} ×${powTxt(1.6,l)} → ×${powTxt(1.6,n)}`, apply:(m,l)=>m.floorLoot*=Math.pow(1.6,l)},
 {id:'s12',sp:'spoils',     nm:{ko:'전리품 감식',en:"Spoils Appraisal"}, max:Infinity,c:l=>24*Math.pow(2.5,l),
  d:(l,n=l+1)=>`${X('보스 보상',"Boss rewards")} ×${powTxt(1.8,l)} → ×${powTxt(1.8,n)}`, apply:(m,l)=>m.boss*=Math.pow(1.8,l)},
 {id:'s13',sp:'starshard',     nm:{ko:'별의 조각',en:"Shard of Stars"}, max:Infinity,c:l=>30*Math.pow(2.7,l),
  d:(l,n=l+1)=>`${X('모든 생산·획득',"All output")} ×${powTxt(1.25,l)} → ×${powTxt(1.25,n)}`,
 ...
 {id:'s14',sp:'route',      nm:{ko:'항로 기록',en:"Charted Routes"}, max:Infinity,c:l=>26*Math.pow(2.45,l),
  d:(l,n=l+1)=>`${X('탐사 깊이 배율',"Depth multiplier")} ×${powTxt(1.3,l)} → ×${powTxt(1.3,n)}`, apply:(m,l)=>m.floorPct*=Math.pow(1.3,l)},
 {id:'s15',sp:'unsealed', nm:{ko:'금서 해방',en:"Unsealed Tome"}, max:Infinity,c:l=>34*Math.pow(2.8,l),
  d:(l,n=l+1)=>`${X('상위 시설 효율',"Higher tiers")} ×${powTxt(1.7,l)} → ×${powTxt(1.7,n)}`, apply:(m,l)=>m.tUp*=Math.pow(1.7,l)},
 {id:'s16',sp:'soulforge',    nm:{ko:'혼의 벼림',en:"Soulforge"}, max:Infinity,c:l=>40*Math.pow(2.9,l),
  d:(l,n=l+1)=>`${X('장비 효과 지수',"Gear exponent")} ×${powTxt(1.2,l)} → ×${powTxt(1.2,n)}`, apply:(m,l)=>m.gearExp*=Math.pow(1.2,l)},
);

/* 유물 강화 · 승천 통화, 영구 */
export const RELIC_UPS=[
 {id:'a1', sp:'idol',    nm:{ko:'고대 유물',en:"Ancient Relic"},  max:Infinity,c:l=>1*Math.pow(3,l),    d:(l,n=l+1)=>`${X('마나 생산',"Mana output")} ×${powTxt(2,l)} → ×${powTxt(2,n)}`, apply:(m,l)=>m.prod*=Math.pow(2,l)},
 {id:'a2', sp:'pickaxe',  nm:{ko:'영혼 광맥',en:"Soul Vein"},  max:Infinity,c:l=>2*Math.pow(3.2,l),  d:(l,n=l+1)=>`${X('영혼석 획득',"Soul Shards")} ×${powTxt(2.5,l)} → ×${powTxt(2.5,n)}`, apply:(m,l)=>m.soul*=Math.pow(2.5,l)},
 {id:'a3', sp:'clock',nm:{ko:'시간 가속',en:"Time Acceleration"},  max:Infinity,      c:l=>3*Math.pow(4,l),    d:(l,n=l+1)=>`${X('게임 속도',"Game speed")} +${fmt(12*l)}% → +${fmt(12*(n))}%`, apply:(m,l)=>m.speed*=1+0.12*l},
 {id:'a4', sp:'voyage',  nm:{ko:'심연 항해',en:"Abyssal Voyage"},  max:Infinity,c:l=>4*Math.pow(3.4,l),  d:(l,n=l+1)=>`${X('던전 보상',"Dungeon rewards")} ×${powTxt(3,l)} → ×${powTxt(3,n)}`, apply:(m,l)=>m.floorLoot*=Math.pow(3,l)},
 {id:'a5', sp:'blessing',  nm:{ko:'왕국의 축복',en:"Kingdom's Blessing"},max:Infinity,c:l=>5*Math.pow(3.5,l),  d:(l,n=l+1)=>X(`환생 후 각 시설 ${fmt(freeFrom(freeRaw()))}개 → ${fmt(freeFrom(freeRaw()+8))}개 무료`,`Free buildings after rebirth: ${fmt(freeFrom(freeRaw()))} → ${fmt(freeFrom(freeRaw()+8))}`), apply:()=>{}},
 {id:'a6', sp:'banner',      nm:{ko:'심연의 지도',en:"Map of the Abyss"},max:Infinity,c:l=>6*Math.pow(3.8,l),  d:(l,n=l+1)=>`${X('층당 배율',"Depth bonus")} +${fmt(2*l)}%p → +${fmt(2*(n))}%p`, apply:(m,l)=>m.floorPct+=0.02*l},
 {id:'a7', sp:'engrave',nm:{ko:'룬 각인술',en:"Rune Inscription"}, max:Infinity,       c:l=>10*Math.pow(4.5,l), d:(l,n=l+1)=>`${X('룬 최대 레벨',"Rune level cap")} +${fmt(10*l)} → +${fmt(10*(n))}`, apply:(m,l)=>m.runeCap+=10*l},
 {id:'a8', sp:'treasure',      nm:{ko:'보물 감식',en:"Treasure Appraisal"},  max:Infinity,c:l=>8*Math.pow(3.6,l),  d:(l,n=l+1)=>`${X('결정 획득',"Crystals")} ×${powTxt(2,l)} → ×${powTxt(2,n)}`, apply:(m,l)=>m.crystal*=Math.pow(2,l)},
 {id:'a9', sp:'chain',    nm:{ko:'시련의 인장',en:"Seal of Trials"},max:Infinity,c:l=>12*Math.pow(4,l),   d:(l,n=l+1)=>`${X('도전 보상',"Trial rewards")} ×${powTxt(1.5,l)} → ×${powTxt(1.5,n)}`, apply:(m,l)=>m.chalPow*=Math.pow(1.5,l)},
 {id:'a10',sp:'seed',     nm:{ko:'초월의 씨앗',en:"Seed of Transcendence"},max:Infinity,c:l=>20*Math.pow(5,l),   d:(l,n=l+1)=>`${X('모든 생산·획득',"All output")} ×${powTxt(1.35,l)} → ×${powTxt(1.35,n)}`, apply:(m,l)=>{const v=Math.pow(1.35,l);m.prod*=v;m.soul*=v;m.offer*=v;m.crystal*=v;m.dungeon*=v;}},
];

RELIC_UPS.push(
 {id:'a11',sp:'beacon', nm:{ko:'심연의 등대',en:"Abyssal Beacon"}, max:Infinity,c:l=>7*Math.pow(3.4,l),
  d:(l,n=l+1)=>`${X('탐사 깊이 배율',"Depth multiplier")} ×${powTxt(1.8,l)} → ×${powTxt(1.8,n)}`,
  apply:(m,l)=>m.floorPct*=Math.pow(1.8,l)},
 {id:'a12',sp:'forge',   nm:{ko:'불멸의 벼림',en:"Undying Forge"}, max:Infinity,c:l=>9*Math.pow(3.7,l),
  d:(l,n=l+1)=>`${X('장비 효과 지수',"Gear exponent")} ×${powTxt(1.35,l)} → ×${powTxt(1.35,n)}`,
  apply:(m,l)=>m.gearExp*=Math.pow(1.35,l)},
 {id:'a13',sp:'eternalhand', nm:{ko:'영원한 손길',en:"Eternal Hands"}, max:Infinity,c:l=>11*Math.pow(3.9,l),
  d:(l,n=l+1)=>`${X('자동화 주기',"Automation interval")} ×${cutTxt(0.85,l)} → ×${cutTxt(0.85,n)}`,
...
export const STAR_UPS=[
 {id:'t1', sp:'starsigil',     nm:{ko:'별의 인장',en:"Star Sigil"},     max:Infinity,c:l=>1*Math.pow(3,l),
  d:(l,n=l+1)=>`${X('모든 생산·획득',"All output")} ×${powTxt(2,l)} → ×${powTxt(2,n)}`,
...
export const ETER_UPS=[
 {id:'e1', sp:'etersigil', nm:{ko:'영원의 각인',en:"Eternal Sigil"}, max:Infinity,c:l=>1*Math.pow(2.5,l),
  d:(l,n=l+1)=>`${X('모든 생산·획득',"All output")} ×${powTxt(10,l)} → ×${powTxt(10,n)}`,
 ...
 {id:'e2', sp:'timecrown',nm:{ko:'시간의 지배',en:"Mastery of Time"}, max:Infinity,c:l=>2*Math.pow(3,l),
  d:(l,n=l+1)=>`${X('게임 속도',"Game speed")} ×${powTxt(2,l)} → ×${powTxt(2,n)}`,
 ...
 {id:'e3', sp:'abysscrown', nm:{ko:'심연의 지배',en:"Dominion of the Abyss"}, max:Infinity,c:l=>2*Math.pow(2.8,l),
  d:(l,n=l+1)=>`${X('던전 공격력',"Dungeon power")} ×${powTxt(20,l)} → ×${powTxt(20,n)}`,
  apply:(m,l)=>m.dungeon*=Math.pow(20,l)},
 {id:'e4', sp:'agespoils', nm:{ko:'영겁의 전리품',en:"Spoils of Ages"}, max:Infinity,c:l=>3*Math.pow(3,l),
  d:(l,n=l+1)=>`${X('던전·보스 보상',"Dungeon & boss rewards")} ×${powTxt(8,l)} → ×${powTxt(8,n)}`,
 ...
 {id:'e5', sp:'omniforge',    nm:{ko:'전지의 벼림',en:"Omniscient Forge"}, max:Infinity,c:l=>4*Math.pow(3.4,l),
  d:(l,n=l+1)=>`${X('장비 효과 지수',"Gear exponent")} ×${powTxt(2,l)} → ×${powTxt(2,n)}`,
  apply:(m,l)=>m.gearExp*=Math.pow(2,l)},
 {id:'e6', sp:'etercrown',nm:{ko:'영원의 왕관',en:"Eternal Crown"}, max:Infinity,c:l=>4*Math.pow(3.2,l),
  d:(l,n=l+1)=>`${X('룬 최대 레벨',"Rune level cap")} +${fmt(200*l)} → +${fmt(200*(n))}`, apply:(m,l)=>m.runeCap+=200*l},
 {id:'e7', sp:'nulleconomy',   nm:{ko:'무의 경제',en:"Economy of Nothing"}, max:Infinity,c:l=>5*Math.pow(3.6,l),
  d:(l,n=l+1)=>`${X('남는 시설 비용',"Building cost left")} ${cutTxt(0.7,l)} → ${cutTxt(0.7,n)}`,
 ...
 {id:'e8', sp:'eternight',    nm:{ko:'잠들지 않는 영원',en:"Sleepless Eternity"}, max:Infinity,c:l=>3*Math.pow(2.6,l),
  d:(l,n=l+1)=>`${X('오프라인 상한',"Offline cap")} +${fmt(24*l)}${X('시간','h')} → +${fmt(24*(n))}${X('시간','h')}`,
  apply:(m,l)=>m.offline+=24*l},
];
ETER_UPS.push(
 {id:'e9', sp:'eonring',   nm:{ko:'영겁의 고리',en:"Ring of Aeons"}, max:Infinity,c:l=>4*Math.pow(3,l),
  d:(l,n=l+1)=>`${X('모든 생산·획득',"All output")} ×${powTxt(6,l)} → ×${powTxt(6,n)}`,
...
export const INF_UPS=[
 {id:'i1', sp:'inf_core',  nm:{ko:'무한의 핵',en:"Infinite Core"}, max:Infinity,c:l=>1*Math.pow(2.2,l),
  d:(l,n=l+1)=>`${X('모든 생산·획득',"All output")} ×${powTxt(4,l)} → ×${powTxt(4,n)}`,
 ...
 {id:'i2', sp:'inf_prism', nm:{ko:'무한 프리즘',en:"Infinite Prism"}, max:Infinity,c:l=>1*Math.pow(2.4,l),
  d:(l,n=l+1)=>`${X('마나 생산',"Mana output")} ×${powTxt(8,l)} → ×${powTxt(8,n)}`, apply:(m,l)=>m.prod*=Math.pow(8,l)},
 {id:'i3', sp:'inf_cross', nm:{ko:'교차하는 길',en:"Crossing Paths"}, max:Infinity,c:l=>2*Math.pow(2.6,l),
  d:(l,n=l+1)=>`${X('던전 공격력',"Dungeon power")} ×${powTxt(10,l)} → ×${powTxt(10,n)}`, apply:(m,l)=>m.dungeon*=Math.pow(10,l)},
 {id:'i4', sp:'inf_disc',  nm:{ko:'무한 원반',en:"Infinite Disc"}, max:Infinity,c:l=>2*Math.pow(2.5,l),
  d:(l,n=l+1)=>`${X('탐사 깊이 배율',"Depth multiplier")} ×${powTxt(4,l)} → ×${powTxt(4,n)}`, apply:(m,l)=>m.floorPct*=Math.pow(4,l)},
 {id:'i5', sp:'inf_burst', nm:{ko:'터지는 빛',en:"Bursting Light"}, max:Infinity,c:l=>3*Math.pow(2.7,l),
  d:(l,n=l+1)=>`${X('던전·보스 보상',"Dungeon & boss rewards")} ×${powTxt(5,l)} → ×${powTxt(5,n)}`,
 ...
 {id:'i6', sp:'inf_glass', nm:{ko:'모래시계 유리',en:"Hourglass Pane"}, max:Infinity,c:l=>3*Math.pow(3,l),
  d:(l,n=l+1)=>`${X('게임 속도',"Game speed")} ×${powTxt(1.5,l)} → ×${powTxt(1.5,n)}`, apply:(m,l)=>m.speed*=Math.pow(1.5,l)},
 {id:'i7', sp:'inf_orbit', nm:{ko:'무한 궤도',en:"Infinite Orbit"}, max:Infinity,c:l=>3*Math.pow(2.8,l),
  d:(l,n=l+1)=>`${X('영혼석·오퍼링 획득',"Soul Shards & Offerings")} ×${powTxt(6,l)} → ×${powTxt(6,n)}`,
...
export const REAL_UPS=[
 ...
 {id:'r2', sp:'real_cube',  nm:{ko:'세계의 정육면체',en:"World Cube"}, max:Infinity,c:l=>2*Math.pow(2.8,l),
  d:(l,n=l+1)=>`${X('마나 생산',"Mana output")} ×${powTxt(80,l)} → ×${powTxt(80,n)}`, apply:(m,l)=>m.prod*=Math.pow(80,l)},
 {id:'r3', sp:'real_wave',  nm:{ko:'실재의 파동',en:"Wave of Being"}, max:Infinity,c:l=>2*Math.pow(3,l),
  d:(l,n=l+1)=>`${X('게임 속도',"Game speed")} ×${powTxt(3,l)} → ×${powTxt(3,n)}`, apply:(m,l)=>m.speed*=Math.pow(3,l)},
 {id:'r4', sp:'real_eye',   nm:{ko:'현실을 보는 눈',en:"Eye of Reality"}, max:Infinity,c:l=>3*Math.pow(3.1,l),
  d:(l,n=l+1)=>`${X('던전 공격력',"Dungeon power")} ×${powTxt(60,l)} → ×${powTxt(60,n)}`, apply:(m,l)=>m.dungeon*=Math.pow(60,l)},
 {id:'r5', sp:'real_pillar',nm:{ko:'세계의 기둥',en:"World Pillar"}, max:Infinity,c:l=>3*Math.pow(2.9,l),
  d:(l,n=l+1)=>`${X('상위 시설 효율',"Higher tiers")} ×${powTxt(10,l)} → ×${powTxt(10,n)}`, apply:(m,l)=>m.tUp*=Math.pow(10,l)},
 {id:'r6', sp:'real_spiral',nm:{ko:'회귀의 나선',en:"Spiral of Return"}, max:Infinity,c:l=>4*Math.pow(3.2,l),
  d:(l,n=l+1)=>`${X('자동화 주기',"Automation interval")} ×${cutTxt(0.5,l)} → ×${cutTxt(0.5,n)}`,
  apply:(m,l)=>m.autoSpeed*=Math.pow(0.5,l)},
 {id:'r7', sp:'real_shard', nm:{ko:'현실 파편',en:"Reality Shard"}, max:Infinity,c:l=>4*Math.pow(3,l),
  d:(l,n=l+1)=>`${X('결정 획득',"Crystals")} ×${powTxt(30,l)} → ×${powTxt(30,n)}`, apply:(m,l)=>m.crystal*=Math.pow(30,l)},
 {id:'r8', sp:'real_net',   nm:{ko:'인과의 그물',en:"Net of Causes"}, max:Infinity,c:l=>5*Math.pow(3.3,l),
  d:(l,n=l+1)=>`${X('도전 보상',"Trial rewards")} ×${powTxt(4,l)} → ×${powTxt(4,n)}`, apply:(m,l)=>m.chalPow*=Math.pow(4,l)},
 {id:'r9', sp:'real_bloom', nm:{ko:'만개하는 세계',en:"Blooming World"}, max:Infinity,c:l=>5*Math.pow(3.4,l),
  d:(l,n=l+1)=>`${X('환생 후 각 시설',"Free buildings after rebirth:")} ${fmt(freeFrom(freeRaw()))} → ${fmt(freeFrom(freeRaw()+5000))}${X('개 무료',' free')}`, apply:()=>{}},
 {id:'r10',sp:'real_key',   nm:{ko:'현실의 열쇠',en:"Key of Reality"}, max:Infinity,c:l=>6*Math.pow(3.6,l),
  d:(l,n=l+1)=>`${X('승천 유물 획득',"Relics on ascension")} ×${powTxt(10,l)} → ×${powTxt(10,n)}`, apply:(m,l)=>m.relic*=Math.pow(10,l)},
];
export const VOID_UPS=[
 {id:'v1', sp:'void_maw',   nm:{ko:'공허의 아가리',en:"Maw of the Void"}, max:Infinity,c:l=>1*Math.pow(3,l),
  d:(l,n=l+1)=>`${X('모든 생산·획득',"All output")} ×${powTxt(200,l)} → ×${powTxt(200,n)}`,
 ...
 {id:'v2', sp:'void_rift',  nm:{ko:'공허의 균열',en:"Void Rift"}, max:Infinity,c:l=>2*Math.pow(3.2,l),
  d:(l,n=l+1)=>`${X('탐사 깊이 배율',"Depth multiplier")} ×${powTxt(40,l)} → ×${powTxt(40,n)}`, apply:(m,l)=>m.floorPct*=Math.pow(40,l)},
 {id:'v3', sp:'void_hand',  nm:{ko:'삼키는 손',en:"Swallowing Hand"}, max:Infinity,c:l=>2*Math.pow(3.1,l),
  d:(l,n=l+1)=>`${X('남는 시설 비용',"Building cost left")} ${cutTxt(0.5,l)} → ${cutTxt(0.5,n)}`,
  apply:(m,l)=>m.costMul*=Math.pow(0.5,l)},
 {id:'v4', sp:'void_crown', nm:{ko:'공허의 관',en:"Crown of the Void"}, max:Infinity,c:l=>3*Math.pow(3.4,l),
  d:(l,n=l+1)=>`${X('보스 보상',"Boss rewards")} ×${powTxt(100,l)} → ×${powTxt(100,n)}`, apply:(m,l)=>m.boss*=Math.pow(100,l)},
 {id:'v5', sp:'void_chain', nm:{ko:'끊긴 사슬',en:"Severed Chain"}, max:Infinity,c:l=>3*Math.pow(3.3,l),
  d:(l,n=l+1)=>`${X('도전 보상',"Trial rewards")} ×${powTxt(8,l)} → ×${powTxt(8,n)}`, apply:(m,l)=>m.chalPow*=Math.pow(8,l)},
 {id:'v6', sp:'void_star',  nm:{ko:'꺼진 별',en:"Dead Star"}, max:Infinity,c:l=>4*Math.pow(3.5,l),
  d:(l,n=l+1)=>`${X('영혼석 획득',"Soul Shards")} ×${powTxt(150,l)} → ×${powTxt(150,n)}`, apply:(m,l)=>m.soul*=Math.pow(150,l)},
 {id:'v7', sp:'void_gate',  nm:{ko:'없음의 문',en:"Door of Nothing"}, max:Infinity,c:l=>4*Math.pow(3.6,l),
  d:(l,n=l+1)=>`${X('게임 속도',"Game speed")} ×${powTxt(5,l)} → ×${powTxt(5,n)}`, apply:(m,l)=>m.speed*=Math.pow(5,l)},
 {id:'v8', sp:'void_tear',  nm:{ko:'공허의 눈물',en:"Tear of the Void"}, max:Infinity,c:l=>5*Math.pow(3.8,l),
  d:(l,n=l+1)=>`${X('룬 최대 레벨',"Rune level cap")} +${fmt(2000*l)} → +${fmt(2000*(n))}`, apply:(m,l)=>m.runeCap+=2000*l},
];
export const ORIGIN_UPS=[
 {id:'o1', sp:'orig_seed',  nm:{ko:'근원의 씨',en:"Seed of Origin"}, max:Infinity,c:l=>1*Math.pow(3.5,l),
  d:(l,n=l+1)=>`${X('모든 생산·획득',"All output")} ×${powTxt(2000,l)} → ×${powTxt(2000,n)}`,
 ...
 {id:'o2', sp:'orig_flame', nm:{ko:'첫 불꽃',en:"First Flame"}, max:Infinity,c:l=>2*Math.pow(3.6,l),
  d:(l,n=l+1)=>`${X('마나 생산',"Mana output")} ×${powTxt(5000,l)} → ×${powTxt(5000,n)}`, apply:(m,l)=>m.prod*=Math.pow(5000,l)},
 {id:'o3', sp:'orig_tree',  nm:{ko:'세계수',en:"World Tree"}, max:Infinity,c:l=>2*Math.pow(3.7,l),
  d:(l,n=l+1)=>`${X('아래 두 단계 효율',"Bottom two tiers")} ×${powTxt(500,l)} → ×${powTxt(500,n)}`, apply:(m,l)=>m.t0*=Math.pow(500,l)},
 {id:'o4', sp:'orig_eye',   nm:{ko:'처음을 본 눈',en:"Eye That Saw First"}, max:Infinity,c:l=>3*Math.pow(3.8,l),
  d:(l,n=l+1)=>`${X('던전 공격력',"Dungeon power")} ×${powTxt(1000,l)} → ×${powTxt(1000,n)}`, apply:(m,l)=>m.dungeon*=Math.pow(1000,l)},
 {id:'o5', sp:'orig_crown', nm:{ko:'근원의 왕관',en:"Crown of Origin"}, max:Infinity,c:l=>4*Math.pow(4,l),
  d:(l,n=l+1)=>`${X('게임 속도',"Game speed")} ×${powTxt(10,l)} → ×${powTxt(10,n)}`, apply:(m,l)=>m.speed*=Math.pow(10,l)},
 {id:'o6', sp:'orig_all',   nm:{ko:'모든 것의 시작',en:"Beginning of All"}, max:Infinity,c:l=>6*Math.pow(4.5,l),
  d:(l,n=l+1)=>`${X('환생 후 각 시설',"Free buildings after rebirth:")} ${fmt(freeFrom(freeRaw()))} → ${fmt(freeFrom(freeRaw()+1e5))}${X('개 무료',' free')}`, apply:()=>{}},
];

/* 도전 · 승천 1회 후 해금. 제약을 걸고 목표 마나 달성 시 영구 보상 */
export const CHALLENGES=[
 {id:'c1', at:'ascend',sp:'sealedbook',   nm:{ko:'침묵의 시련',en:"Trial of Silence"}, rule:{noResearch:1}, base:1e9,  max:100,
  desc:()=>X('연구를 사용할 수 없다',"Research cannot be used"),            rw:c=>`${X('마나 생산',"Mana output")} ×${powTxt(1.8,c)}`, apply:(m,c)=>m.prod*=Math.pow(1.8,c)},
 {id:'c2', at:'ascend',sp:'emptypouch',   nm:{ko:'빈곤의 시련',en:"Trial of Poverty"}, rule:{maxTier:2},    base:1e8,  max:100,
 ...
 {id:'c3', at:'ascend',sp:'brokencog',    nm:{ko:'고독의 시련',en:"Trial of Solitude"}, rule:{noAuto:1},     base:1e10, max:100,
  desc:()=>X('모든 자동화가 멈춘다',"All automation halts"),             rw:c=>`${X('자동화 주기',"Automation interval")} ×${cutTxt(0.9,c)}`, apply:(m,c)=>m.autoSpeed*=Math.pow(0.9,c)},
 {id:'c4', at:'ascend',sp:'darkeye',  nm:{ko:'어둠의 시련',en:"Trial of Darkness"}, rule:{noDungeon:1},  base:1e12, max:100,
  desc:()=>X('던전에 들어갈 수 없다',"The dungeon is sealed"),            rw:c=>`${X('던전 공격력',"Dungeon power")} ×${powTxt(2.2,c)}`, apply:(m,c)=>m.dungeon*=Math.pow(2.2,c)},
 {id:'c5', at:'ascend',sp:'crackedvial',   nm:{ko:'고갈의 시련',en:"Trial of Drought"}, rule:{drain:1e3},    base:1e7,  max:100,
 ...
 {id:'c6', at:'ascend',sp:'abysseye',  nm:{ko:'심연의 시련',en:"Trial of the Abyss"}, rule:{noResearch:1,noAuto:1,noDungeon:1,drain:10}, base:1e13, max:100,
...
CHALLENGES.push(
 {id:'c7', at:'trans',sp:'torpor',nm:{ko:'무기력의 시련',en:"Trial of Torpor"}, rule:{noAuto:1,drain:100,slow:3}, base:1e11, max:100,
 ...
 {id:'c8', at:'trans',sp:'narrow',nm:{ko:'좁은 길의 시련',en:"Trial of the Narrow Path"}, rule:{maxTier:1}, base:1e7, max:100,
 ...
 {id:'c9', at:'trans',sp:'weight',nm:{ko:'무게의 시련',en:"Trial of Weight"}, rule:{maxTier:3,noAuto:1}, base:1e9, max:100,
 ...
 {id:'c10', at:'trans',sp:'blind',nm:{ko:'눈먼 시련',en:"Trial of the Blind"}, rule:{noDungeon:1,noResearch:1}, base:1e12, max:100,
 ...
 {id:'c11', at:'trans',sp:'silentstar',nm:{ko:'침묵하는 별',en:"The Silent Star"}, rule:{noResearch:1,noRelicGear:1}, base:1e13, max:100,
 ...
 {id:'c12', at:'inf',sp:'origintrial',nm:{ko:'근원의 시련',en:"Trial of the Origin"},
...
CHALLENGES.push(
 {id:'c13', at:'inf',sp:'dryspring',nm:{ko:'메마른 샘',en:"The Dry Spring"}, rule:{drain:1e6}, base:1e10, max:100,
 ...
 {id:'c14', at:'inf',sp:'lone',nm:{ko:'홀로 선 자',en:"The Lone Apprentice"}, rule:{maxTier:0}, base:1e6, max:100,
 ...
 {id:'c15', at:'inf',sp:'cogstop',nm:{ko:'멈춘 톱니',en:"Seized Gears"}, rule:{noAuto:1,noResearch:1}, base:1e11, max:100,
  desc:()=>X('자동화도 연구도 없다',"No automation, no research"),
  rw:c=>`${X('자동화 주기',"Automation interval")} ×${cutTxt(0.88,c)}`, apply:(m,c)=>m.autoSpeed*=Math.pow(0.88,c)},
 {id:'c16', at:'inf',sp:'sealedvein',nm:{ko:'닫힌 광맥',en:"The Sealed Vein"}, rule:{noDungeon:1,drain:1e3}, base:1e9, max:100,
 ...
 {id:'c17', at:'eter',sp:'barehand',nm:{ko:'맨손의 시련',en:"Bare Hands"}, rule:{noRelicGear:1,noAuto:1}, base:1e10, max:100,
  desc:()=>X('룬·장비가 봉인되고 자동화도 멈춘다',"Runes and gear sealed, no automation"),
  rw:c=>`${X('장비 효과 지수',"Gear exponent")} ×${powTxt(1.5,c)}`, apply:(m,c)=>m.gearExp*=Math.pow(1.5,c)},
 {id:'c18', at:'eter',sp:'dryaltar',nm:{ko:'마른 제단',en:"The Dry Altar"}, rule:{maxTier:4,noDungeon:1}, base:1e12, max:100,
 ...
 {id:'c19', at:'eter',sp:'withersoul',nm:{ko:'혼이 마르는 곳',en:"Where Souls Wither"}, rule:{noResearch:1,drain:1e8}, base:1e13, max:100,
 ...
 {id:'c20', at:'eter',sp:'nothing',nm:{ko:'무의 시련',en:"Trial of Nothing"},
...
CHALLENGES.push(
 {id:'c21', at:'real', sp:'sealedvein', nm:{ko:'닫힌 심연',en:"The Closed Abyss"},
  rule:{noDungeon:1,noRelicGear:1}, base:1e17, max:100,
  desc:()=>X('던전도 룬·장비도 없다',"No dungeon, no runes or gear"),
  rw:c=>`${X('영혼석 획득',"Soul Shards")} ×${powTxt(1.6,c)}`, apply:(m,c)=>m.soul*=Math.pow(1.6,c)},
 {id:'c22', at:'real', sp:'dryspring', nm:{ko:'마르지 않는 갈증',en:"Unquenched"},
  rule:{drain:1e9,noResearch:1}, base:1e18, max:100,
  desc:()=>X('마나가 끝없이 새고 연구도 못 한다',"Mana drains without end; no research"),
  rw:c=>`${X('결정 획득',"Crystals")} ×${powTxt(1.7,c)}`, apply:(m,c)=>m.crystal*=Math.pow(1.7,c)},
 {id:'c23', at:'void', sp:'lone', nm:{ko:'홀로 남은 탑',en:"The Last Tower"},
  rule:{maxTier:0,noAuto:1}, base:1e19, max:100,
  desc:()=>X('견습 마법사 하나로, 손수',"One apprentice, by hand"),
  rw:c=>`${X('오퍼링 획득',"Offerings")} ×${powTxt(1.8,c)}`, apply:(m,c)=>m.offer*=Math.pow(1.8,c)},
 {id:'c24', at:'void', sp:'blind', nm:{ko:'보이지 않는 심연',en:"The Unseen Deep"},
  rule:{noDungeon:1,noResearch:1,noAuto:1}, base:1e20, max:100,
  desc:()=>X('던전도 연구도 자동화도 없다',"No dungeon, no research, no automation"),
  rw:c=>`${X('유물 획득',"Relics")} ×${powTxt(1.5,c)}`, apply:(m,c)=>m.relic*=Math.pow(1.5,c)},
 {id:'c25', at:'origin', sp:'weight', nm:{ko:'근원의 무게',en:"Weight of Origin"},
  rule:{maxTier:2,noRelicGear:1,drain:1e6}, base:1e21, max:100,
  desc:()=>X('시설도 룬·장비도 막히고 마나가 샌다',"Buildings capped, runes and gear sealed, mana drains"),
  rw:c=>`${X('던전 보상',"Dungeon rewards")} ×${powTxt(1.9,c)}`, apply:(m,c)=>m.floorLoot*=Math.pow(1.9,c)},
 {id:'c26', at:'origin', sp:'cogstop', nm:{ko:'멈춘 근원',en:"The Stilled Origin"},
  rule:{noAuto:1,noResearch:1,slow:5,drain:1e7}, base:1e22, max:100,
  desc:()=>X('모든 것이 멈추고 느려진다',"Everything halts and slows"),
  rw:c=>`${X('보스 보상',"Boss rewards")} ×${powTxt(2.2,c)}`, apply:(m,c)=>m.boss*=Math.pow(2.2,c)},
);
...
/* ── 항목마다 다른 재료 ──────────────────────────────
   한 갈래의 스무 항목이 모두 같은 재료를 먹으면, 무엇을 먼저 올릴지는 값의 크기
   하나로만 갈린다 — 재료가 여섯 가지든 열 가지든 고르는 일은 한 가지뿐이다.
   항목마다 다른 재료를 요구하면 '지금 무엇이 모자란가' 가 선택이 된다.
   그리고 구하기 어려운 것을 요구하는 항목은 한 레벨이 그만큼 크다. 값과 몫이
   같이 움직여야 어려운 재료를 내주는 뜻이 생긴다.
   pow 는 레벨에 곱해진다 — 이 게임의 곱셈 강화는 전부 base^(레벨에 선형인 식)
   꼴이라, 레벨을 키우는 것이 곧 몫을 그만큼 키우는 것이다(foldUp 이 기대는 성질). */
export const UP_MAT_SETS={
  soul:  [['soul',1],['crystal',1.10],['offering',1.22]],
  relic: [['relic',1],['crystal',1.10],['soul',1.22],['offering',1.34]],
  star:  [['star',1],['relic',1.10],['soul',1.22],['crystal',1.34]],
  inf:   [['inf',1],['star',1.12],['relic',1.26]],
  eter:  [['eter',1],['inf',1.12],['star',1.26]],
  real:  [['real',1],['eter',1.12],['inf',1.26]],
  void:  [['void',1],['real',1.12],['eter',1.26]],
  origin:[['origin',1],['void',1.12],['real',1.26]],
};
export function assignMats(defs,key){
  const list=UP_MAT_SETS[key]; if(!list) return;
  /* 첫 항목은 언제나 그 갈래의 제 재료다 — 처음 열리는 하나가 낯선 재료를
     요구하면 시작부터 막힌다. 그 뒤로 돌아가며 나눈다. */
  defs.forEach((u,i)=>{ const [mat,pow]=list[i%list.length]; u.mat=mat; u.pow=pow; });
}
export function matOf(u,curKey){ return (u&&u.mat)||curKey }
export function powOf(u){ const p=u&&u.pow; return (typeof p==='number'&&p>0&&isFinite(p))?p:1 }
assignMats(SOUL_UPS,'soul');
assignMats(RELIC_UPS,'relic');
assignMats(STAR_UPS,'star');
assignMats(INF_UPS,'inf');
assignMats(ETER_UPS,'eter');
assignMats(REAL_UPS,'real');
assignMats(VOID_UPS,'void');
assignMats(ORIGIN_UPS,'origin');
